...
const COMPANY_WALLET = env_1.config.blockchain.companyWallet.address;
...
describe('BlockchainService', () => {
    it('should verify a valid address', () => {
        const result = blockchainService_1.BlockchainService.isValidAddress('0x742d35Cc6634C0532925a3b844Bc454e4438f44e', 'ETH');
        expect(result).toBe(true);
    });
    it('should transfer crypto successfully', async () => {
        const txHash = await blockchainService_1.BlockchainService.transferCrypto('0x742d35Cc6634C0532925a3b844Bc454e4438f44e', '0.01', 'ETH');
...
async function runBlockchainTests() {
    ...
    try {
        ...
        try {
            const ethBalance = await balanceService_1.BalanceService.getWalletBalance(TEST_ADDRESS, "ETH");
        ...
        try {
            const usdtBalance = await balanceService_1.BalanceService.getWalletBalance(TEST_ADDRESS, "USDT");
        ...
        try {
            const companyEthBalance = await balanceService_1.BalanceService.getWalletBalance(COMPANY_WALLET, "ETH");
        ...
        try {
            ...
            const gasEstimate = await blockchainService_1.BlockchainService.estimateGas(TEST_ADDRESS, "0.001", "ETH");
        ...
        try {
            const isVerified = await blockchainService_1.BlockchainService.verifyTransaction(testTxHash, "ETH");
        ...
        try {
            const swapEstimate = await blockchainService_1.BlockchainService.getSwapEstimate("0.01", "ETH", "USDT");
