...
async function testSellFlow() {
    try {
        ...
        const nairaAmount = await priceService_1.PriceService.convertCryptoToNaira(cryptoAmount, cryptoType);
        ...
        // Process bank payout
        const payoutResult = await korapayService_1.KorapayService.processBankPayout(nairaAmount, accountNumber, selectedBank.code, accountName, `Crypto sale: ${cryptoAmount} ${cryptoType}`, reference);
        ...
        const statusResult = await korapayService_1.KorapayService.checkPayoutStatus(payoutResult.reference);
        console.log('\nCurrent status:');
...
testSellFlow();
//# sourceMappingURL=test-sell-flow.js.map