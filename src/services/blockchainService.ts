...
export class BlockchainService {
  private static provider: ethers.Provider = new ethers.JsonRpcProvider(
    "https://eth-mainnet.g.alchemy.com/v2/_9Cg-dFoye2kHGgkOHajuOWCVGiO0_m1"
  );
  ...
  static async getBalance(
    ...
    try {
      ...
      throw new Error(
        `BlockchainError: Blockchain operation failed: ${
  ...
  static async getTronsBalance(tronAddress: string, coin: "USDT" | "WETH") {
    try {
      const TRC20_CONTRACTS = {
        USDT: "TXLAQ63Xg1NAzckPwKHvzw7CSEmLMEqcdj", // USDT TRC-20 contract address
        WETH: "TKkeiboTkxXKJpbmVFbv4a8ov5rAfRDMf9", // WETH TRC-20 contract address
      };
      // Get the contract address for the specified coin
      const contractAddress = TRC20_CONTRACTS[coin];
      if (!contractAddress) {
      ...
      // Convert the balance from SUN to human-readable format (USDT/WETH have 6 decimal places)
  ...
  static async isValidAddress(
    ...
    try {
      switch (cryptoType.toUpperCase()) {
        ...
        case "TRX":
          return await WalletService.verifyTronAddress(address);
        case "BTC":
          // Basic Bitcoin address validation (this is a simplified check)
          const btcRegex =
            config.blockchain.network === "mainnet"
    ...
    try {
      ...
      if (cryptoType === "ETH") {
        ...
        if (balance < amountWei) {
          throw new Error(
            `Insufficient balance. Available: ${ethers.formatEther(
              balance
            )} ${cryptoType}, Required: ${amount} ${cryptoType}`
          );
        }
        ...
        const connectedContract = tokenContract.connect(
          wallet
        ) as IERC20Contract;
        ...
        if (tokenBalance < tokenAmount) {
          throw new Error(
            `Insufficient ${cryptoType} balance. Available: ${ethers.formatUnits(
              tokenBalance,
              decimals
            )} ${cryptoType}, Required: ${amount} ${cryptoType}`
          );
        ...
        console.log(
          `Sending ${amount} ${cryptoType} to ${toAddress} with gas limit ${gasLimit}`
        );
        ...
        const tronWeb = new TronWeb({
          fullHost: 'https://api.trongrid.io', // Tron network node
          privateKey: fromPrivateKey, // Use the provided private key
        });
        ...
        console.log('Sender balance:', tronWeb.fromSun(balanceInSun), 'TRX');
  
        // Check if sender has enough balance
        if (balanceInSun < amountInSun) {
          throw new Error(
            `Insufficient balance. Available: ${tronWeb.fromSun(
              balanceInSun
            )} TRX, Required: ${amount} TRX`
          );
        }
  ...
  static async validateTransaction(
    ...
    try {
      console.log(
        `Validating transaction ${transactionHash} for ${amount} ${cryptoType}`
      );

      const ethersProvider = this.getEthersProvider(cryptoType);

      // Get transaction details from the blockchain
      const txReceipt = await ethersProvider.getTransactionReceipt(
        transactionHash
      );
      ...
      if (cryptoType === "ETH") {
        ...
        const contractAddress =
          typeof tokenContract.target === "string"
  ...
  static async swapCrypto(
    ...
    try {
      ...
      if (
        (fromCrypto !== "ETH" && fromCrypto !== "USDT") ||
      ...
      if (fromCrypto === "ETH" && toCrypto === "USDT") {
        ...
        const minOutputAmount =
          (expectedOutputAmount * BigInt(slippageFactor)) / 1000n;

        console.log(
          `Expected USDT output: ${ethers.formatUnits(expectedOutputAmount, 6)}`
        );
        console.log(
          `Minimum USDT output (with ${slippagePercentage}% slippage): ${ethers.formatUnits(
            minOutputAmount,
            6
          )}`
        );
        ...
        if (usdtBalance < amountIn) {
          throw new Error(
            `Insufficient USDT balance. Available: ${ethers.formatUnits(
              usdtBalance,
              decimals
            )} USDT`
          );
        ...
        console.log(
          `Expected ETH output: ${ethers.formatEther(expectedOutputAmount)}`
        );
        console.log(
          `Minimum ETH output (with ${slippagePercentage}% slippage): ${ethers.formatEther(
            minOutputAmount
          )}`
        );
        ...
        const approveTx = await connectedUsdtContract.approve(
          routerAddress,
          amountIn
        );
      ...
      throw new Error(
        `Failed to swap ${fromCrypto} to ${toCrypto}: ${
          error instanceof Error ? error.message : "Unknown error"
        }`
      );
    }
    ...
    try {
      ...
      try {
        if (fromCrypto === "ETH" && toCrypto === "USDT") {
          ...
          // Create swap path: ETH -> WETH -> USDT
          const path = [
            this.WETH_ADDRESS[network],
            this.USDT_CONTRACT_ADDRESS[network],
          ];
          ...
          throw new Error(
            `Unsupported swap pair: ${fromCrypto} to ${toCrypto}`
          );
        }
      } catch (error: unknown) {
        ...
        if (
          ...
          console.warn(
            `Insufficient liquidity for ${fromCrypto} to ${toCrypto} swap on testnet`
          );
  ...
  static async getWalletBalance(
    ...
    try {
      ...
      console.error(
        `Error getting ${cryptoType} balance for ${address}:`,
        error
      );
      throw new Error(
  ...
  static async getBitcoinBalance(address: string): Promise<string> {
    try {
      ...
      const network =
        config.blockchain.network === "mainnet" ? "main" : "test3";
      ...
      const totalBalance =
        (balanceSatoshis + unconfirmedBalanceSatoshis) / 100000000;
    ...
    try {
      ...
      if (cryptoType === "ETH") {
        ...
        const wallet = new ethers.Wallet(
          BlockchainService.COMPANY_WALLET_PRIVATE_KEY,
          provider
        );
        ...
        if (balance < amountInWei) {
          throw new Error(
            `Insufficient balance in company wallet: ${ethers.formatEther(
              balance
            )} ETH`
          );
        }
        ...
        if (tokenBalance < tokenAmount) {
          throw new Error(
            `Insufficient USDT balance in company wallet: ${ethers.formatUnits(
              tokenBalance,
              decimals
            )} USDT`
          );
        ...
        // Send token transfer transaction
        console.log(
          `Sending ${amount} USDT to ${toAddress} with gas limit ${gasLimit}`
        );
        const transaction = await connectedContract.transfer(
          toAddress,
          tokenAmount,
  ...
  static async transferTRX(toAddress: string, amount: string): Promise<string> {
    try {
      console.log("=== TRON TRANSFER INITIATED ===");
      console.log("To address:", toAddress);
      console.log("Amount:", amount);

      // Initialize TronWeb (replace with your TronWeb configuration)

      // Validate the address
      if (!tronWeb.isAddress(toAddress)) {
        throw new Error(`Invalid TRX address: ${toAddress}`);
      }
      ...
      console.log(
        "Company wallet balance:",
        tronWeb.fromSun(balanceInSun),
        "TRX"
      );

      // Check if we have enough balance
      if (Number(balanceInSun) < Number(amountInSun)) {
        throw new Error(
          `Insufficient balance in company wallet: ${tronWeb.fromSun(
            balanceInSun
          )} TRX`
        );
      }
      ...
      const transferOption = {
        privateKey: BlockchainService.COMPANY_WALLET_PRIVATE_KEY,
        address: BlockchainService.COMPANY_WALLET_ADDRESS,
      };
      ...
      console.log("Transaction sent:", transaction.txid);
      console.log("=== TRON TRANSFER COMPLETED ===");
  ...
  static async transferFromUserToCompany(
    ...
    try {
      ...
      if (cryptoType === "ETH") {
        ...
        if (balance < amountWei) {
          throw new Error(
            `Insufficient ETH balance. Required: ${amount}, Available: ${ethers.formatEther(
              balance
            )}`
          );
        }
        ...
        const gasCost =
          gasLimit * (gasPrice.gasPrice ?? ethers.parseUnits("50", "gwei"));

        // Ensure user has enough ETH for gas
        if (balance < amountWei + gasCost) {
          throw new Error(
            `Insufficient ETH balance for gas. Required: ${amount} + gas, Available: ${ethers.formatEther(
              balance
            )}`
          );
        }
        ...
        const connectedContract = usdtContract.connect(
          wallet
        ) as IERC20Contract;
        ...
        if (balance < tokenAmount) {
          throw new Error(
            `Insufficient USDT balance. Required: ${amount}, Available: ${ethers.formatUnits(
              balance,
              decimals
            )}`
          );
  ...
  static async createWallet(
    ...
    try {
      ...
      return {
        address: `0x${crypto_type}_${Math.random()
          .toString(36)
          .substring(2, 15)}`,
  ...
  static async sendBitcoin(
    ...
    try {
      ...
      const utxos = utxoResponse.data.txrefs.map((utxo: any) => {
        return {
          ...
          script: new bitcore.Script(
            new bitcore.Address(fromAddress, network)
          ).toHex(),
          satoshis: utxo.value,
        };
      });
      ...
      const totalBalance = utxos.reduce(
        (sum: number, utxo: any) => sum + utxo.satoshis,
        0
      );

      if (totalBalance < satoshis) {
        throw new Error(
          `Insufficient balance. Required: ${amount} BTC, Available: ${
